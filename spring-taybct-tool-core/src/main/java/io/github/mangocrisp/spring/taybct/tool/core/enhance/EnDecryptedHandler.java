...
public class EnDecryptedHandler implements IMethodEnhanceHandler {
     ...
     * @return 返回处理后的字符中
     */
    private static String enDecryptedStr(@Nullable String argumentsName
            , String str
            , boolean isParameter
        ...
        String edStr = str;
        // 获取处理方法
        Function<String, String>[] enDeHandlerFunctions = getEnDeHandlerFunctions(argumentsName
                , isParameter
                , enDecryptedMethodSupplier
                , enDecryptedElementSupplier);
        if (enDeHandlerFunctions != null) {
            for (Function<String, String> enDeHandlerFunction : enDeHandlerFunctions) {
...
//        }
    ...
    private static void enDeMap(Map<String, Object> edMap
        ...
        for (String key : keyMap.keySet()) {
            String edStr = (String) keyMap.get(key);
            Function<String, String>[] enDeHandlerFunctions = functionMap.get(key);
            if (enDeHandlerFunctions != null) {
                for (Function<String, String> enDeHandlerFunction : enDeHandlerFunctions) {
                    edStr = enDeHandlerFunction.apply(edStr);
                }
     ...
     * @param enDecryptedMethodSupplier  如何获取到 {@link EnhanceElements} 注解，比如是方法上的，或者是类型上的
     * @param enDecryptedElementSupplier 如何获取到 {@link EnhanceElement} 注解，比如是方法参数上的，或者是类型字段上的
     * @return {@code Function<String, String>[]} 处理方法数组，按数组顺序处理增强
     */
...
//                    }
    ...
    private static void getMapEnDeHandlerFunctions(Map<String, Object> edMap
        ...
        // 每个参数名会用到的增强器
        Map<String, Function<String, String>[]> enDecryptedValueMap = new HashMap<>();
        if (enDecryptedMethod != null) {

            // 这个是最外层的处理方式，如果里面不指定默认用这个
            Function<String, String>[] enDeHandlerFunctions = getEnDeHandlerFunctions(isParameter ? enDecryptedMethod.parameterHandler()
            ...
            if (StringUtil.isBlank(argumentsName)) {
                ...
                for (EnhanceElementMap enhanceElementMap : enDecryptedMethod.map()) {
                    ...
                    Function<String, String>[] mapFunctions = getEnDeHandlerFunctions(enhanceElementMap.resultHandler());
        ...
        if (enDecryptedElement != null) {
            ...
            Function<String, String>[] enDeHandlerFunctions = getEnDeHandlerFunctions(isParameter ? enDecryptedElement.parameterHandler()
                    : enDecryptedElement.resultHandler());

            for (EnhanceElementMap enhanceElementMap : enDecryptedElement.map()) {
                String[] keyArr = enhanceElementMap.value();
                Function<String, String>[] mapFunctions = getEnDeHandlerFunctions(isParameter ? enhanceElementMap.parameterHandler()
     ...
     */
    @Nullable
    private static Function<String, String>[] getEnDeHandlerFunctions(@Nullable String argumentsName
            , boolean isParameter
            , Supplier<EnhanceElements> enDecryptedMethodSupplier
            , Supplier<EnhanceElement> enDecryptedElementSupplier) {
        EnhanceElements enDecryptedMethod = enDecryptedMethodSupplier.get();
        // 每个参数会用到的增强器
        Map<String, Function<String, String>[]> enDecryptedParameterMap = new HashMap<>();
        // 每个参数名会用到的增强器
        Map<String, Function<String, String>[]> enDecryptedValueMap = new HashMap<>();
        if (enDecryptedMethod != null) {
            if (StringUtil.isBlank(argumentsName)) {
                ...
                Class<? extends Function<String, String>>[] classes = enDecryptedMethod.resultHandler();
            ...
            for (EnhanceElement enDecryptedElement : enDecryptedMethod.enDecryptedElements()) {
                ...
                Class<? extends Function<String, String>>[] classes = isParameter ? enDecryptedElement.parameterHandler()
                        : enDecryptedElement.resultHandler();
                Function<String, String>[] enDeHandlerFunctions = getEnDeHandlerFunctions(classes);
                if (enDeHandlerFunctions != null) {
                    enDecryptedParameterMap.put(key, enDeHandlerFunctions);
                }
            }
            for (String key : enDecryptedMethod.value()) {
                Class<? extends Function<String, String>>[] classes = isParameter ? enDecryptedMethod.parameterHandler()
                        : enDecryptedMethod.resultHandler();
                Function<String, String>[] enDeHandlerFunctions = getEnDeHandlerFunctions(classes);
                if (enDeHandlerFunctions != null) {
     ...
     */
    @Nullable
    private static Function<String, String>[] getEnDeHandlerFunctions(Class<? extends Function<String, String>>[] classes) {
        Function<String, String>[] array = null;
        List<Function<String, String>> funList = new ArrayList<>();
        for (Class<? extends Function<String, String>> clazz : classes) {
            try {
                Constructor<? extends Function<String, String>> declaredConstructor = clazz.getDeclaredConstructor();
                Function<String, String> function = declaredConstructor.newInstance();

...
//        User u2 = (User) dh.query("SELECT id,name,age,address FROM t_user WHERE id=?", n);
    ...
    public static void printSql(PreparedStatement ps) {
        if (ps instanceof DruidPooledPreparedStatement) {
            DruidPooledPreparedStatement druidPs = (DruidPooledPreparedStatement) ps;
            PreparedStatement rawPreparedStatement = druidPs.getRawPreparedStatement();
            if (rawPreparedStatement instanceof PreparedStatementProxyImpl) {
                PreparedStatementProxyImpl cstmt = (PreparedStatementProxyImpl) rawPreparedStatement;
